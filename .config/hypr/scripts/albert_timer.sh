...
prompt() {
  ...
  if has_cmd rofi; then
    rofi -dmenu -p "$message" -theme-str 'entry { placeholder: "Type here"; }' <<<"$value"
  else
    printf '%s' "$value"
  fi
}
...
start_timer() {
  ...
  task_input="$(prompt 'Task for this round' '')"
  [[ -z "${task_input// }" ]] && exit 0

  minutes_input="$(prompt 'Minutes to spare' "$DEFAULT_MINUTES")"
